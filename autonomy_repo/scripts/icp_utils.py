...
def nearest_neighbor(src: np.ndarray, dst: np.ndarray, radius=0.01):
    ...
    neighAlgo = NearestNeighbors(n_neighbors=1, radius=radius)
    neighAlgo.fit(dst)
    distances, indices = neighAlgo.kneighbors(src) 
    ########### YOUR CODE ENDS HERE ###########
...
def icp(
    ...
    for i in trange(max_iterations):
        ...
        src_prime = R @ src + t.reshape(4, 1)
        err = np.mean(src_prime - src)
