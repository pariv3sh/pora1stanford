...
class RRT(object):
    ...
    def compute_path(self, P, V):
        self.path = [self.x_goal]
        cidx = self.goal_idx
        while cidx != 0:
            pidx = P[cidx]
            self.path.append(V[pidx])
            cidx = pidx
        self.path.reverse()
    ...
    def solve(self, eps, max_iters=1000, goal_bias=0.05, shortcut=False):
        ...
        success = False
        xinit, xgoal = self.x_init, self.x_goal
        sslo, sshigh = self.statespace_lo, self.statespace_hi

        for k in range(max_iters):
            #draw from a uniform distribution
            z = uniform.rvs(loc=0, scale=1, size=1)[0]
            rand2d = uniform.rvs(loc=sslo, scale=sshigh, size=state_dim)
            xrand = xgoal if z < goal_bias else rand2d
            
            xnear_i = self.find_nearest(V, xrand)
            xnear = V[xnear_i]

            #print(f'{z=} {xnear_i=}{xrand=}{xgoal=}')
            xnext = self.steer_towards(xnear, xrand, eps)
            #print(f'{xnext=}')
            if self.is_free_motion(self.obstacles, xnear, xnext):
                V[n] = xnext # 
                P[n] = xnear_i
                n += 1
                #print(xnext)
                #print(xgoal)
                if np.array_equal(xnext, xgoal):
                    success = True
                    self.goal_idx = n-1
                    break

        self.compute_path(P, V)
        ...
        if success:
            if shortcut:
                ...
                self.shortcut_path()
                self.plot_path(color="orange", linewidth=2, label="Shortcut solution path")
            else:
                self.plot_path(color="green", linewidth=0.5, label="Solution path")
    ...
    def shortcut_path(self):
        ...
        while not success:
            ...
            for i, x in enumerate(self.path):
                ...
                px, cx = self.path[i-1], self.path[i+1]
                
                if self.is_free_motion(self.obstacles, px, cx):
                    del self.path[i] 
                    success = False
